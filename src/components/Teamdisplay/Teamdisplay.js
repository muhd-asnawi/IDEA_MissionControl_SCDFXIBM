...
function createData(name, status, timein) {
  return {name, status, timein};
}
...
export default function Teamdisplay(props) {
    ...
    props.data.map(person => data.push(createData(<Button onClick = {() => props.select(person.name)}>{person.name}</Button>, person.status, person.timein)));
